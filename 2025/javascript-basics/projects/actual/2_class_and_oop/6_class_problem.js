...
class MaleIdol extends Idol {
    static fromObject(object) { //나의 풀이에서만 필요
        return new MaleIdol(
            object.name,
            object.year,
        );
...
// 나의 풀이
const iveMembersClass = iveMembers.map((x) => FemaleIdol.fromObject(x));
console.log(iveMembersClass);

const ive = new IdolGroup('아이브', iveMembersClass);
console.log(ive);

const btsMembersClass = btsMembers.map((x) => MaleIdol.fromObject(x));
console.log(btsMembersClass);

const bts = new IdolGroup('BTS', btsMembersClass);
console.log(bts);
